...
public:
	...
	// std::vector<int_vector<sigma_bits>> m_compressed_text_per_superblock;
...
public:
	explicit rank_support_int_v(const int_vector<vector_width>* v = nullptr) : rank_support_int<alphabet_size,vector_width>(v)
	{
        ...
        for (auto & node : nodes)
        {
            // Store blocks_per_superblock - 1 many blocks for every letter except the last one.
            // The first block for every superblock is always 0 and does not need to be stored.
            // The count for the biggest letter in sigma is always 1, so we don't need to store this.
            node.blocks.width(new_width);
            node.blocks.resize((blocks_per_superblock - 1) * max_letter, 0);
            node.superblocks.resize(max_letter, 0);
        }
        ...
        for (auto node_it = nodes.begin(); node_it != nodes.end(); ++node_it)
        {
            ...
            auto text_slice_end = std::next(text_slice_it,
                                            std::min<size_t>(std::distance(text_slice_it, v->end()),
                                                             values_per_superblock));
            node_it->text_slice.assign(text_slice_it, text_slice_end);
            text_slice_it = text_slice_end; // Move the text iterator to the next slice begin.

            // Second initialise the superblock counts: buf_blocks corresponds to
            for (size_t letter_rank = 0; letter_rank < max_letter; ++letter_rank)
            {
                buf_superblocks[letter_rank] += buf_blocks[letter_rank]; // Update sum with previous superblock
                node_it->superblocks[letter_rank] = buf_superblocks[letter_rank]; // Store the counts.
            ...
            for (auto block_it = node_it->blocks.begin(); word_id < word_count && block_it != node_it->blocks.end(); ++word_id)
            {
                ...
                for (size_t letter_rank = 0; letter_rank < max_letter; ++letter_rank, ++block_it)
                {
    ...
    //             << "\n";
        ...
        uint64_t const word = *(node.text_slice.data() + (bit_pos / this->bits_per_word));
	...
	size_type prefix_rank2(const size_type idx, const value_type v) const
	{
        ...
        auto superblock_it = node.superblocks.begin() + (v - 1); // Move iterator to the predecessor of v.
        ...
        auto block_it = node.blocks.begin() + pppp;
...
{
    int_vector<0> blocks;
    int_vector<64> superblocks;
    int_vector<sigma_bits> text_slice;
